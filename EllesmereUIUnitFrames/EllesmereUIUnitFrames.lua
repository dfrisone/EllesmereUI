...
-------------------------------------------------------------------------------
--  Blizzard Style (Global Settings > Style). The stock unit frame look on our -- eui-style: allow comment-budget (style reference)
...
-- The aura block: an invisible frame spanning the stock box's width from the -- eui-style: allow comment-budget (secure layout reference)
...
function ns.UF_BlizzTextPass(frame)
    ...
    lt:ClearAllPoints()
    lt:SetJustifyH("LEFT")
    ns.UF_BlizzPoint(lt, G.name.point, frame, G.name.point, G.name.x, G.name.y, frame._blizzMirror)
    lt:SetWidth(G.name.w)
end

-- Level number in the frame's level circle, as the stock frames draw it: -- eui-style: allow comment-budget (stock layout reference)
...
function InitializeFrames()
        ...
        for _, unitKey in ipairs({"player", "target", "focus"}) do
            ...
            if frame and not ns.VisUnitDisabled(db.profile, unitKey) and s then
                ...
                if frame._euiVisDriver ~= wantDriver and not isLocked then
                    if wantDriver then
                        EllesmereUI.WatchUnitFrame(frame, true)
                        EllesmereUI.SecureCall(RegisterAttributeDriver, frame, "state-visibility", wantDriver)
                    else
                ...
                if mini then
                    ...
                    if mini._euiVisDriver ~= miniWant and not isLocked then
                        if miniWant then
                            EllesmereUI.WatchUnitFrame(mini, true)
                            EllesmereUI.SecureCall(RegisterAttributeDriver, mini, "state-visibility", miniWant)
                        else
                            EllesmereUI.SecureCall(UnregisterAttributeDriver, mini, "state-visibility")
                            EllesmereUI.WatchUnitFrame(mini)
                        end
            ...
            elseif frame then
                ...
                if not isLocked and frame._euiVisDriver and frame._euiVisDriver ~= "hide" then
                    EllesmereUI.SecureCall(RegisterAttributeDriver, frame, "state-visibility", "hide")
                ...
                if mini then
                    ...
                    if miniAlways then
                        ...
                        if not isLocked and mini._euiVisDriver and mini._euiVisDriver ~= "hide" then
                            EllesmereUI.SecureCall(RegisterAttributeDriver, mini, "state-visibility", "hide")
    ...
    if not frames._visFrame then
        ...
        if not EUI_IS_FOREVER then frames._visFrame:RegisterEvent("PLAYER_CAN_GLIDE_CHANGED") end
...
function SetupOptionsPanel()
    ...
    function ns.UF_SetBossFramesActive(on)
        ...
        for i = 1, 5 do
            ...
            if f then
                if on then
                    EllesmereUI.WatchUnitFrame(f)
                else
                    EllesmereUI.WatchUnitFrame(f, true)
                    f:Hide()
                end
            end
        end
