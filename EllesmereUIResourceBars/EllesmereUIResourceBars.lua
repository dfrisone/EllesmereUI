...
-- Namespaced first: the loose global is gone on newer clients and this alias feeds every
-- spec lookup in the file.
...
local function BuildBarTypeSpecMap()
    -- All four, not just the first. The spec globals moved into C_SpecializationInfo and
    -- the loose copies are gone on newer clients, so guarding one of them only moved the
    -- error a line down. Nothing spec-shaped exists to map where they are missing.
    if not (GetNumClasses and GetClassInfo and GetNumSpecializationsForClassID
        and GetSpecializationInfoForClassID) then return end
...
--  atlases on our own frame: every feature (timer, latency, ticks, pips, kick -- eui-style: allow thirdparty-maybe (Blizzard texture atlas)
...
-- The frame around a bar: one texture on the host under everything the bar -- eui-style: allow comment-budget (atlas geometry reference)
...
function ERB:ApplyAll()
    ...
    if not ERB._vehicleProxy then
        local function InitVehicleProxy()
            ...
            EllesmereUI.SecureCall(ERB._vehicleProxy.SetAttribute, ERB._vehicleProxy, "_onstate-erbvehicle", [[
            ...
            EllesmereUI.SecureCall(RegisterStateDriver, ERB._vehicleProxy, "erbvehicle", "[vehicleui][petbattle] hide; show")
        end
...
function ERB:OnEnable()
    ...
    if not EUI_IS_FOREVER then eventFrame:RegisterEvent("PLAYER_CAN_GLIDE_CHANGED") end
