...
-------------------------------------------------------------------------------
--  Blizzard Style (Global Settings > Style). The stock nameplate atlases on -- eui-style: allow comment-budget (style reference)
...
function ns.GetIconBorderEnabled(kind)
    local key
    if kind == "cast" then
        -- Blizzard Style: the stock cast icon has no border.
        if ns.NP_Blizz() then return false end
