...
--       "spell activation overlay glow" on a spell we're tracking).
...
-------------------------------------------------------------------------------
--  Unlock Mode registration -- three independently movable/resizable
--  elements, grouped with the rest of Quality of Life.
-------------------------------------------------------------------------------
C_Timer.After(2, function()
    ...
    local function MakeMoverEntry(key, label, order, isHiddenKey, getFrameFn, applyFn, posKey)
        return MK({
            ...
            setWidth = function(_, w)
                local ma = MA()
                ma[posKey] = ma[posKey] or {}
                ma[posKey].width = math.max(60, math.floor(w + 0.5))
                applyFn()
            end,
            setHeight = function(_, h)
                local ma = MA()
                ma[posKey] = ma[posKey] or {}
                ma[posKey].height = math.max(20, math.floor(h + 0.5))
                applyFn()
            end,
            savePos = function(_, point, relPoint, x, y)
                ...
                local ma = MA()
                ma[posKey] = ma[posKey] or {}
                ma[posKey].point, ma[posKey].relPoint, ma[posKey].x, ma[posKey].y = point, relPoint, x, y
            end,
            loadPos = function()
                local pos = MA()[posKey]
    ...
    EllesmereUI:RegisterUnlockElements({
        MakeMoverEntry("EUI_MovementAlert", "Movement Alert", 750, "enabled", function() return movementFrame end, ApplyMovementFrame, "pos"),
        MakeMoverEntry("EUI_TimeSpiralAlert", "Movement Alert - Time Spiral", 751, "tsEnabled", function() return timeSpiralFrame end, ApplyTimeSpiralFrame, "tsPos"),
        MakeMoverEntry("EUI_GatewayShardAlert", "Movement Alert - Gateway Shard", 752, "gwEnabled", function() return gatewayFrame end, ApplyGatewayFrame, "gwPos"),
    })
end)
